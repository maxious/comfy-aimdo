...
static inline bool mod1(ModelVBAR *mv, size_t page_nr, bool do_free, bool do_unpin) {
    ...
    char *vaddr = (char *)mv->vbar + page_nr * VBAR_PAGE_SIZE;

    do_free = do_free && rp->handle && (do_unpin || !rp->pinned);
    if (do_free) {
        CHECK_ZE(zeVirtualMemUnmap(aimdo_xpu_ctx, vaddr, VBAR_PAGE_SIZE));
        CHECK_ZE(zePhysicalMemDestroy(aimdo_xpu_ctx, rp->handle));
...
size_t vbars_free(size_t size) {
    ...
    for (ModelVBAR *i = lowest_priority.higher; pages_needed && i != &highest_priority;
        ...
        for (;pages_needed && i->watermark > i->watermark_limit; i->watermark--) {
            if (!dirty) {
                CHECK_ZE(zeDeviceSynchronize(aimdo_xpu_dev));
                dirty = true;
            }
...
void *vbar_allocate(uint64_t size, int device) {
    ...
    if (!CHECK_ZE(zeVirtualMemReserve(aimdo_xpu_ctx, NULL, size, &mv->vbar))) {
        log(ERROR, "Could not reserve Virtual Address space for VBAR\n");
        free(mv);
        return NULL;
    }
...
int vbar_fault(void *vbar, uint64_t offset, uint64_t size, uint32_t *signature) {
    ...
    for (uint64_t page_nr = VBAR_GET_PAGE_NR(offset); page_nr < page_end; page_nr++) {
        ze_result_t err = ZE_RESULT_ERROR_OUT_OF_DEVICE_MEMORY;
        char *vaddr = (char *)mv->vbar + page_nr * VBAR_PAGE_SIZE;
        ResidentPage *rp = &mv->residency_map[page_nr];
        ...
        if (budget_deficit(VBAR_PAGE_SIZE) ||
            (err = three_stooges(aimdo_xpu_dev, aimdo_xpu_ctx, vaddr, VBAR_PAGE_SIZE, mv->device, &rp->handle)) != ZE_RESULT_SUCCESS) {
            ...
            if ((err = three_stooges(aimdo_xpu_dev, aimdo_xpu_ctx, vaddr, VBAR_PAGE_SIZE, mv->device, &rp->handle)) != ZE_RESULT_SUCCESS) {
...
void vbar_free(void *vbar) {
    ...
    remove_vbar(mv);
    CHECK_ZE(zeVirtualMemFree(aimdo_xpu_ctx, mv->vbar, (size_t)mv->nr_pages * VBAR_PAGE_SIZE));
    free(mv);
}
